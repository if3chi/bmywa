    ...
    <div class="max-w-5xl mx-auto">
        <div class="flex flex-col items-center md:flex-row">
            ...
            <x-entry.form wire:submit.prevent="submitEntry">
                ...
                <x-input.group wire:model="editing.firstname" type="text" :error="$errors->first('editing.firstname')"
                    placeholder="Ama" id="firstname" label="First Name" />
                <x-input.group wire:model="editing.lastname" type="text" :error="$errors->first('editing.lastname')"
                    placeholder="Kwaku" id="lastname" label="Last Name" />
                <x-input.group type="email" wire:model="editing.email" :error="$errors->first('editing.email')"
                    placeholder="AmaKwaku@gmail.com" id="email" label="Email Address" />
                <x-input.group type="phone" wire:model="editing.phone" :error="$errors->first('editing.phone')"
                    placeholder="0201234567" id="phone" label="Phone" />


                <x-input.select wire:model="editing.entry_type" id="entry-type" label="Entry Type"
                ...
                <div class="flex mb-6 w-full flex-col relative">
                    <div class="flex">
                        <x-input.number wire:model="editing.age" id="age" min="6" max="15" label="Age" placeholder="7"
                            :error="$errors->first('editing.age')" />
                        <x-input.ref wire:model="editing.entry_fee" label="Entry Fee Ref"
                            :error="$errors->first('editing.entry_fee')" placeholder="02#4567" />
                    </div>
                </div>

                <x-input.textarea wire:model="editing.award_entry" id="award-entry" type="text" label="Award Entry"
